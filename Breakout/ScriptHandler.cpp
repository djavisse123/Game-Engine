...
void ScriptHandler::onEvent(Event e) {
    switch(e.type) {
        case Event::DECREASE_SPEED_SCRIPT:
        {
            Character *spriteObj = (Character*) parentObject;
            {
                ...
                //Add the slow character movement script to the manager
                sm->addScript("slow_character_movement", "scripts/slow_character_movement.js", "sprite_context");
                //Expose the sprite to V8 so it can be modified
                spriteObj->exposeToV8(isolate, sprite_context, "sprite_context");
                //Run the slow character movement script to modify the sprite's speed
                sm->runOne("slow_character_movement", reload, "sprite_context");
